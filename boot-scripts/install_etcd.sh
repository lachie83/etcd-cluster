...
if [ "true" = "$IS_LEADER" ]; then
    ...
    while ./etcdctl --endpoint "http://${cluster_name}.${dns_zone}:2379" cluster-health 2>&1 | grep -q -e "exceeded header timeout" -e "failed to list members" -e "etcd cluster is unavailable or misconfigured"; then 
